...
import finalforeach.cosmicreach.worldgen.generators.MoonZoneGenerator;
import me.zombii.horizon.NostalgicIslandZoneGenerator;
...
//        BlockState waterBlock = BlockState.getInstance("base:water[default]");
    ...
    public WorldCube() {
        ...
        ZoneGenerator generator = new MoonZoneGenerator();

        world = PhysicsZone.create(uuid, generator);
...
//                for (int y = 0; y < 20; y++) {
                    for (int z = 0; z < 10; z++) {
                        ChunkColumn chunkColumn = new ChunkColumn(x, 0, z);
                        generator.generateForChunkColumn(world, chunkColumn);
                    }
//                }
            }
    ...
    public void render(Camera worldCamera) {
        ...
        if (modelInstance != null) {
            if (worldCamera.frustum.boundsInFrustum(this.globalBoundingBox)) {
                ...
                modelInstance.render(Gdx.graphics.getDeltaTime(), this, worldCamera, tmpModelMatrix, true);
