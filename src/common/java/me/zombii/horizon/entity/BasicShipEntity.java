...
public class BasicShipEntity extends Entity implements IPhysicEntity, IVirtualZoneEntity {
    ...
    public BasicShipEntity() {
        ...
        if (GameSingletons.isHost) {
            ...
            PhysicsChunk structure00 = new PhysicsChunk(new Vec3i(1, 0, 1));
            PhysicsChunk structure01 = new PhysicsChunk(new Vec3i(0, 0, 1));
            PhysicsChunk structure10 = new PhysicsChunk(new Vec3i(1, 0, 0));
            PhysicsChunk structure11 = new PhysicsChunk(new Vec3i(0, 0, 0));
    ...
    public void update(Zone zone, float deltaTime) {
        ...
        MatrixUtil.rotateAroundOrigin3(oBoundingBox, transform, new Vector3(
                -oBoundingBox.getBounds().getWidth() / 2,
                -oBoundingBox.getBounds().getWidth() / 2,
                -oBoundingBox.getBounds().getWidth() / 2
        ), rotation);
    ...
    public void render(Camera worldCamera) {
        ...
        if (worldCamera.frustum.boundsInFrustum(this.globalBoundingBox)) {
            ...
            if (modelInstance != null) {
                modelInstance.render(.05f, this, worldCamera, tmpModelMatrix, true);
            }
        }
    }
